...
def is_single_circuit(junction_boxes):
    first_circuit = junction_boxes[0]['circuit']
    return all(box['circuit'] == first_circuit for box in junction_boxes)
...
if __name__ == "__main__":
    ...
    while not single_circuit:
        closest_pair = (edges[connections][1], edges[connections][2])
        connect_junction_boxes(junction_boxes, closest_pair)
        connections += 1
        single_circuit = is_single_circuit(junction_boxes)
        if single_circuit:
            x1 = junction_boxes[closest_pair[0]]['x']
            x2 = junction_boxes[closest_pair[1]]['x']
            dist = x1*x2
            print(f"All boxes connected after {connections} connections. Distance metric: {dist}")
